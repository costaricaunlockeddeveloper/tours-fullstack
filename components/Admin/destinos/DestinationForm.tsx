...
export default function DestinationForm({ initialData, onSubmit, isSubmitting, onCancel, simpleMode = false }: DestinationFormProps) {
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        // In simple mode, auto-generate slug if missing
        if (!formData.slug && formData.name) {
            const autoSlug = formData.name.toLowerCase().replace(/ /g, "-").replace(/[^a-z0-9-]/g, "");
            // We'll let the setFormData update happen, but for submit use the calculated one
