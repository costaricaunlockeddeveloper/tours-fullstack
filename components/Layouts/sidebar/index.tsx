...
export function Sidebar() {
  ...
  const filteredNavData = NAV_DATA.map(section => {
    return {
      ...
      items: section.items.filter(item => {
        ...
        if (userRole === "client") {
          return ["Dashboard", "Mis Reservas", "Perfil"].includes(item.title);
        }
        // If not logged in or loading, probably only show Dashboard?
  ...
  }, [pathname, filteredNavData]); // Added filteredNavData dependency for safety
        ...
        <div className="flex h-full flex-col py-10 pl-[25px] pr-[7px]">
          <div className="relative pr-4.5">
            <Link
              ...
              className="flex justify-center px-0 py-2.5 min-[850px]:py-0"
            >
              <Logo />
            </Link>
